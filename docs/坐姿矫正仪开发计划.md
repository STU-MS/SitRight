...
第一版协议故意做得极简，避免硬件和软件两边联调时浪费时间。推荐格式：
...
这是最容易在 Arduino 和 C# 两边快速跑通的协议。其优点是：人眼可读、串口调试助手可直接观察、解析逻辑极简、容错好。对第一版而言没必要一开始就做复杂帧头、校验和、多字段。真正需要增强时，再扩展为：
...
但第一版不建议这么做，因为收益不大、复杂度上升。

串口参数建议默认：`115200 8N1`。虽然你的数据量很小，9600 也够，但 115200 在桌面侧缓冲和响应上更常见，且未来如果加调试信息也更从容。上位机需要支持配置 COM 口名、波特率，并在连接后显示“设备在线、最近值、最后接收时间、错误次数”。
